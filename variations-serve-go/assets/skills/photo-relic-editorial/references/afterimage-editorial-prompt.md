...
Create a two-part editorial artwork from the user's photograph whose layout follows the source orientation. The finished image should feel like a distinctive Photo Relic series: the real photograph kept intact on one side, and on the other a restrained modern print made from the photo's memory. The relic should be recognizable but compressed, artful but not decorative, quiet but not weak.
...
- one photographic region that reproduces the supplied image exactly — pixel-truthful, full-bleed, uncropped, no border, frame, margin, or photo-print-on-paper look
- one adjoining relic region of equal size, derived only from the photo
...
The canvas is exactly the photograph doubled along the joining axis: landscape or near-square source → photographic region on top, relic panel below; portrait source → photographic region on the left, relic panel on the right. Never use the other arrangement for the given orientation.

## Prompt Compiler

Before writing the final generation prompt, compile the user's photo into a compact recipe:

- relic placement: centered-field, deep-space, axis-aligned, or horizon-anchored
...
- The photographic region is full-bleed and fills its half of the canvas exactly: no border, frame, white margin, drop shadow, or pasted-photo-on-paper look.
- Do not redraw, crop, or reframe the photo.
...
Create a two-part Photo Relic editorial artwork using the supplied photograph. The canvas is exactly the photograph doubled along the joining axis: for a landscape or near-square source, the photographic region is on top and the relic panel below; for a portrait source, the photographic region is on the left and the relic panel on the right. Use this recipe: [relic placement] / [relic grammar] / [mark weight] / [title mode] / [motion seed]. Reproduce the photograph in its region exactly as supplied — pixel-truthful, full-bleed, uncropped, unredrawn, no border, frame, white margin, or photo-print-on-paper look; no face or object changes, no invented scene content, no beauty filter, and no painterly or illustrative treatment. Pair it with an adjoining warm ivory/off-white paper relic panel of equal size, derived only from the photograph's visible subject identity, light, color, edges, scale, and emotional concept. Build one recognizable primary relic using modern printmaking language: deep ink-like blocks, softened edges, negative-space cuts, sparse trace lines, measured breaks, and at most one small warm source-derived accent. The relic must preserve the full subject relationship with supporting base/path/horizon/scale marks when needed. Keep generous blank space, quiet editorial spacing, and one very small poetic title or no title. The relic panel should feel like memory pressed into paper, not a literal illustration, generic poster, infographic, watercolor copy, UI screenshot, or decorative symbol. For a portrait source, do not stack the photo above the relic on a tall canvas; for a landscape or near-square source, do not place the photo and relic side by side.
...
- Photo truth: the photographic region matches the supplied photograph exactly — full-bleed, uncropped, unredrawn, no border, frame, or margin — and the layout matches the source orientation (stacked for landscape or near-square, side-by-side for portrait).
